...
const SearchDialog = ({ className = '', onClose }: SearchDialogProps) => {
    ...
    return (
        <>
            ...
            <div className="absolute top-1/2 left-1/2 z-30 flex max-h-[550px] w-[640px] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl bg-white p-4 shadow-xl">
                {/* Search input */}
                <div className="flex items-center rounded-lg border-2 border-black/10 bg-gray-200 px-4">
                    ...
                    <TextInput
                        autoFocus
                        className="w-full rounded-none border-none font-mono shadow-none ring-0 outline-none focus:ring-0"
                ...
                >
                    {/* Casts list */}
                    {cast && cast.length > 0 && (
                        <div className="fixed top-0 left-0 flex h-full w-full bg-white/75 backdrop-blur-sm">
                            <div className="m-auto flex h-[90%] w-9/12 flex-col rounded-xl bg-gray-200 p-4">
                                <h3 className="mb-1 flex items-center justify-between text-xl font-bold">
                                    <span>Cast</span>
                                    <span className="cursor-pointer text-xs" onClick={() => setCast(null)}>
                                        Close
                                    </span>
                                </h3>
                                <div className="flex overflow-y-scroll">
                                    <ul className="flex flex-col">
                                        {cast?.map((actor) => (
                                            <li className="">
                                                <span className="font-medium">{actor.original_name}</span> as {actor.character}
                                            </li>
                                        ))}
                                    </ul>
                                </div>
                    ...
                    {results?.data.hits.map((movie) => (
                        <div
                            ...
                            key={movie.id}
                        >
                            <img src={movie.poster_path} alt={movie.original_title} className="h-36 w-28 rounded-lg object-cover" />
                            <div className="flex flex-col gap-2">
                                ...
                                {movie._formatted ? (
                                    <div
                                        ...
                                        dangerouslySetInnerHTML={{
                                            __html: DOMPurify.sanitize(movie._formatted.original_title),
                                        }}
                                    ></div>
                                ) : (
                                    <div className="title gap-2 text-lg font-bold">{movie.original_title}</div>
                                )}
                                ...
                                {movie._formatted ? (
                                    <div
                                        ...
                                        dangerouslySetInnerHTML={{
                                            __html: DOMPurify.sanitize(movie._formatted.overview),
                                        }}
                                    ></div>
                                ) : (
                                    <div className="overview text-sm text-gray-600">{movie.overview}</div>
                                )}

                                <div className="mt-2 flex items-center justify-between">
                                    <span className="text-sm text-gray-600">
                                        Release date: {format(new Date(movie.release_date), 'MMMM d, yyyy')}
                                    </span>

                                    <button
                                        className="rounded-lg bg-gray-200 px-2 py-1 text-xs font-medium shadow-sm hover:bg-gray-300"
                                        onClick={() => showCasts(movie.movieCasts)}
                                    >
                                        View Cast
                                    </button>
                                </div>
                    ...
                    <>
                        <div className="mt-4 flex justify-between border-t border-gray-200 pt-4 font-mono text-xs">
                            <span className="font-medium">
                                <span className="text-gray-500">Total Hits:</span> {results.data.totalHits} results
                            </span>

                            <span className="font-medium">
                                <span className="text-gray-500">Total Pages:</span> {results.data.page} of {results.data.totalPages}
                            </span>
