...
import { CalendarIcon, ChevronRightIcon, ClockIcon, Download, MapPinIcon, QrCodeIcon, SearchIcon, TicketIcon } from 'lucide-react';
...
export default function MyTickets({ tickets }: MyTicketsProps) {
    ...
    const ticketQrCodeData = (id: string) => page.props.ziggy.url + '/tickets/' + id + '/qr';
        ...
        <AppLayout>
            ...
            <div className="p-6 md:py-8">
                <div className="mb-6">
                    ...
                    <p className="text-muted-foreground mt-2">Manage all your purchased tickets</p>
                </div>
                ...
                {tickets.meta.last_page > 1 && (
                    <Pagination className="mt-8">
                        <PaginationContent>
                            ...
                            <PaginationItem>
                                <PaginationNext href={`/my-tickets?page=${Math.min(tickets.meta.last_page, tickets.meta.current_page + 1)}`} />
                            </PaginationItem>
                        </PaginationContent>
                    </Pagination>
                )}
                        ...
                        {selectedTicket && (
                            <div className="flex flex-col items-center justify-center space-y-4 p-4">
                                <div className="rounded-lg border bg-white p-2">
                                    <img
                                        src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=`+ ticketQrCodeData(selectedTicket.id)}
                                        alt="Ticket QR Code"
                                        className="h-48 w-48"
                                    />
                                </div>
...
function TicketsList({ tickets, showQrCode }: { tickets: Ticket[]; showQrCode: (ticket: Ticket) => void }) {
    if (tickets.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-12">
                <TicketIcon className="text-muted-foreground/50 h-12 w-12" />
                <h3 className="mt-4 text-lg font-medium">No tickets found</h3>
                <p className="text-muted-foreground mt-1 text-sm">You don't have any tickets in this category</p>
    ...
    return (
        <div className="space-y-4">
            {tickets.map((ticket) => (
                <TicketCard key={ticket.id} ticket={ticket} showQrCode={showQrCode} />
            ))}
        </div>
    );
}
...
function TicketCard({ ticket, showQrCode }: { ticket: Ticket; showQrCode: (ticket: Ticket) => void }) {
                    ...
                    <div className="flex flex-col p-4 md:p-6">
                        <div className="mb-2 flex items-start justify-between">
                            <div>
                                ...
                                <div className="text-muted-foreground mt-1 flex items-center text-sm">
                                    ...
                                    <span>{ticket.event?.date ? safeFormatDate(ticket.event.date, 'EEE, MMM d, yyyy') : 'Date unavailable'}</span>
                                    {ticket.event?.date && (
                                        <>
                        ...
                        <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-2 text-sm md:grid-cols-3">
                            ...
                            <div>
                                ...
                                <p className="font-medium">{ticket.purchased_at ? safeFormatDate(ticket.purchased_at, 'MMM d, yyyy') : 'N/A'}</p>
                            </div>
                            <div>
                        ...
                        <div className="mt-auto pt-4">
                            <div className="flex flex-wrap gap-2">
                                ...
                                <Button size="sm" onClick={() => window.open(`/tickets/${ticket.id}/pdf`, '_blank')} variant="outline">
